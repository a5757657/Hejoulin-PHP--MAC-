...
<div class="d-flex justify-content-between mt-5">
    <div>
        ...
        <button type="button" class="btn btn-secondary btn-sm"
            onclick="location.href='./gift_detail_insert.php'">新增禮盒資料</button>
    </div>
    <!--這邊是頁數的 Btn  -->
    <nav aria-label="Page navigation example">
        <ul class="pagination">
            ...
            <!-- 跳到最後面的 Btn -->
            <li class="page-item <?= $totalPages == $page ? 'disabled' : '' ?>">
                <a class="page-link" href="?page=<?= $page = $totalPages ?>"><i
...
function delete_it(gift_d_id) {
    ...
    document.querySelector('.modal-footer').innerHTML =
        `<a href="gift_detail_delete.php?gift_d_id=${gift_d_id}" class="btn btn-secondary">刪除</a>`;
    modal.show();
}
